...
- **Vary the opener — and vary the FRAME, not just the words. Never reuse a frame within a fortnight.** A frame is the sentence-shape of the opening, independent of vocabulary. Retired by measurement: **'Most traders…'** (and 'Most people', 'Most retail traders', 'Almost every trader') — it opened roughly half of all posts through July 2026 and now reads as machine-written; and **'I used to think / I used to believe / I thought X… actually Y'**, the confession-reversal frame, which opened three consecutive posts on 28–30 July 2026 the moment 'Most traders' was banned. That is the lesson: ban a phrase and the next crutch grows in its place unless the *shape* is what gets checked. Also retired: 'X doesn't care about Y, it cares about Z' and 'Here's why/what/the thing…' as an opening clause. Rotate deliberately across hook archetypes and pick one the last three posts did not use: a hard number ('8,302 configurations. 98.5% died.'), a first-person admission of a specific cost ('I lost $25,000 learning this.'), a direct question, a flat contrarian statement, a short concrete scene, a named mistake, a before-and-after, or a piece of process stated plainly with no setup.

- **Rotate the material too — a fresh opener on a stale story is still a repeat.** Measured across July 2026's automated Threads output, this material was the most recycled: the 3am/Asia-session level carrying less weight than a London one (6 posts), win rate as a vanity metric vs profit factor (9), the Sentinel post-mortem / '122 files, 22,500 lines' (3), '$25,000 and 8 years' (3), '30 EAs tested' (2), 'two years marking the same zones on XAUUSD' (2), exit logic mattering more than entries (2). None of it is banned — it is good material — but if a piece of it appeared in the recent-posts list, choose different material rather than re-telling the same story from a new angle. The proof library in section 3.1b and the six pillars are deep enough that nothing needs reusing inside a fortnight.
...
- End with one CTA — never two. **One exception:** conversation-starter posts (the single-post Threads slots — roughly half of Threads slots, 11 of 21 weekly, raised from 9 on 31 July 2026) end with a genuine open question and NO CTA at all. Threads weights replies far above likes, and this account's CTA-ended posts have drawn near-zero engagement while its question-shaped posts have carried the whole account. Every other post, on every other channel, still ends with exactly one CTA — and at least one Threads chain runs every day, so no day passes without one.

- **Conversation-starters are SHORT: 1–2 lines total, aim 80–180 characters including the question, 220 absolute ceiling.** Character estimates run ~15% short, so aim at 150 to land under 220 — a 247-character 'short' post went out on 28 July 2026. Casual and human in tone. Measured on this account, posts under ~150 characters average roughly ten times the views of long-form posts, every reader comment the account has ever received came from a short conversational post, and the format's first real conversation (9 comments, 30 July 2026) came from a 168-character one. One narrow relatable idea, then the question — no setup paragraphs.
